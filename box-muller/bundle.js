...
/***/ function(module, exports, __webpack_require__) {
	...
	var _generateData = __webpack_require__(291);
	
	var _generateData2 = _interopRequireDefault(_generateData);
	
	var _renderChart = __webpack_require__(296);
	
	var _renderChart2 = _interopRequireDefault(_renderChart);
	
	var _utils = __webpack_require__(294);
	
	function _interopRequireDefault(obj) { return obj && obj.__esModule ? obj : { default: obj }; }
	
	//import renderGaussian from './renderGaussian.js';
	
	var nInput = document.getElementById('n-input');
	var aInput = document.getElementById('a-input');
	var μInput = document.getElementById('μ-input');
	var σInput = document.getElementById('σ-input');
	
	document.getElementById('refresh-btn').addEventListener('click', generateAndRender);
	
	function generateAndRender() {
	  var n = parseFloat(nInput.value);
	  var a = parseFloat(aInput.value);
	  var μ = parseFloat(μInput.value);
	  var σ = parseFloat(σInput.value);
	
	  var data = (0, _generateData2.default)(n, a, μ, σ);
	  (0, _renderChart2.default)(data, a, μ, σ);
	  //renderGaussian(n, a, μ, σ);
	}
	
	document.getElementById('normal-distribution-btn').addEventListener('click', function () {
	  aInput.value = 1 / Math.sqrt(_utils.TWO_PI);
	  μInput.value = 0.;
	  σInput.value = 1.;
	  generateAndRender();
	});
...
/***/ function(module, exports, __webpack_require__) {
	...
	var _d = __webpack_require__(292);
	
	var _d2 = _interopRequireDefault(_d);
	
	var _boxMullerTransform = __webpack_require__(293);
	
	var _boxMullerTransform2 = _interopRequireDefault(_boxMullerTransform);
	
	var _distributions = __webpack_require__(295);
	
	function _interopRequireDefault(obj) { return obj && obj.__esModule ? obj : { default: obj }; }
	
	function generateData(n, a, μ, σ) {
	  var data = _d2.default.range(n).map(function () {
	    var key = (0, _boxMullerTransform2.default)(μ, σ);
	    var value = (0, _distributions.gaussianDistribution)(key, a, μ, σ);
...
/***/ function(module, exports, __webpack_require__) {
	...
	exports.default = boxMullerTransform;
	
	var _utils = __webpack_require__(294);
	
	function boxMullerTransform(μ, σ) {
	  var x = void 0,
	      y = void 0;
	  do {
	    var u_1 = Math.random();
	    ...
	    if (u_1 === 0) {
	      ...
	      x = Math.sqrt(-2. * Math.log(u_1)) * Math.cos(_utils.TWO_PI * u_2) * σ + μ;
	      y = Math.sqrt(-2. * Math.log(u_1)) * Math.sin(_utils.TWO_PI * u_2) * σ + μ;
	    }
	  } while (!isUniform(x) || !isUniform(y));
	
	  return Math.sqrt(x * x + y * y);
	}
	
	function isUniform(v) {
	  return v >= 0. && v <= 1.;
	}
...
/***/ function(module, exports, __webpack_require__) {
	...
	exports.gaussianDistribution = gaussianDistribution;
	...
	function gaussianDistribution(x, a, μ, σ) {
	  return a / σ * Math.exp(-1. * sqr(x - μ) / (2 * σ * σ));
	}
	
	function normalDistribution(x) {
	  return gaussianDistribution(x, 1 / Math.sqrt(_utils.TWO_PI), 0, 1.);
	}
...
/***/ function(module, exports, __webpack_require__) {
	...
	function renderChart(data, a, μ, σ) {
	  ...
	  y.domain(_d2.default.extent(data, function (d) {
	    return d.value;
	  }));
	
	  var barWidth = width / data.length;
	
	  var chart = _d2.default.select('#chart').attr('width', width + margin.left + margin.right).attr('height', height + margin.top + margin.bottom);
	
	  chart.select('g').remove();
	
	  var chartInner = chart.append('g').attr('transform', 'translate(' + margin.left + ', ' + margin.top + ')');
	
	  chartInner.append('g').attr('class', 'y axis').call(yAxis);
	
	  chartInner.append('g').attr('class', 'x axis').attr('transform', 'translate(0, ' + height + ')').call(xAxis);
	
	  var bars = chartInner.selectAll('g.bar-wrapper').data(data).enter().append('g').attr('transform', function (d, i) {
	  ...
	  */
	  ...
	  chartInner.append('path').datum(data).attr('class', 'line').attr('d', line);
	}
